...
function displayComments() {
  ...
  existingComments.forEach((comment, index) => {
      ...
      comment.replies.forEach((reply, replyIndex) => {
          ...
          `;
          commentElement.querySelector(`#replies-${index}`).appendChild(replyElement);
      });
  ...
  document.querySelectorAll('.toggle-replies').forEach(button => {
      button.addEventListener('click', function () {
          ...
          button.textContent = isVisible ? `${existingComments[index].replies.length} Replies ▼` : `${existingComments[index].replies.length} Replies ▲`;
      });
  });
